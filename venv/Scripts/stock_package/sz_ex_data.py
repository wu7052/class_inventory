...
class sz_web_data(ex_web_data):
    ...
    def basic_info_json_parse(self, json_str=None):
        ...
        if json_str is not None:
            ...
            totalShares = [float(amount)*10000 for amount in ts]  # 换算成 万股 单位
            tfs = jsonpath(json_obj, '$..data..agltgb')  # A股流动资本, 单位 亿股
            totalFlowShares = [float(amount)*10000 for amount in tfs] # 换算成 万股 单位
            ...
            totalPage = jsonpath(json_obj, '$..metadata.pagecount')
            self.total_page = totalPage

            stock_matix = [company_code, company_abbr ,totalShares, totalFlowShares, list_date, company_abbr, industry, industry_code]
